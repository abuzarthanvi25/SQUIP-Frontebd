...
export default function AdminFireBrigade({navigation}) {
  const [fireBrigadeRequests, setFireBrigadeRequests] = useState([]);
  ...
  const getRequests = () => {
    ...
    axios
      ...
      .then(res => {
        ...
        setFireBrigadeRequests(res.data.complains);
        setLoading(false);
      })
      .catch(err => {
        console.log(err);
  ...
  return (
    <>
      <ImageBackground
        ...
        source={{
          uri: 'https://wallpaperset.com/w/full/5/3/f/233452.jpg',
        }}
        ...
        <ScrollView style={{flex: 1}}>
          {loading ? (
            ...
            <View>
              {fireBrigadeRequests && fireBrigadeRequests.length > 0 ? (
                fireBrigadeRequests.map((request, i) => (
